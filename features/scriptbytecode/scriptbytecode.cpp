...
bool TryRobloxSigned(const std::uint8_t* data, size_t n, std::vector<std::uint8_t>& out)
{
	...
	const std::uint32_t got = XXH32(buf.data(), (unsigned)buf.size(), 42u);
	if (got != expect)
		return false;
...
bool SkipProtoBody(Reader& r, std::uint8_t version, ProtoView* capture)
{
	...
	if (!r.VarInt(sizecode) || sizecode > 2000000 || r.Left() < static_cast<size_t>(sizecode) * 4) return false;
	std::vector<std::uint32_t> code;
	...
	if (!r.VarInt(sizek) || sizek > 500000 || r.Left() < sizek) return false;
...
std::string DecompileLuau(const std::vector<std::uint8_t>& bc, const char* name)
{
	...
	if (!r.VarInt(string_count) || string_count > 2000000 || string_count > r.Left())
	{
...
{
	out.clear();
	if (script_addr == 0 || Memory == nullptr || !IsScriptClass(class_name))
		return false;

	const uintptr_t bc_obj = Memory->read<uintptr_t>(script_addr + ByteCodeOffset(class_name));
	if (bc_obj == 0)
		return false;

	const uintptr_t ptr = Memory->read<uintptr_t>(bc_obj + Offsets::ByteCode::Pointer);
	const uint64_t size = Memory->read<uint64_t>(bc_obj + Offsets::ByteCode::Size);
	if (ptr == 0 || size == 0 || size > (16u * 1024u * 1024u))
		return false;

	out.resize(static_cast<size_t>(size));
	Memory->readRaw(ptr, out.data(), static_cast<uintptr_t>(size));
	return out.size() > 0;
}

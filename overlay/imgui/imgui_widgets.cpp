...
#pragma warning (disable: 26812)    // [Static Analyzer] The enum type 'xxx' is unscoped. Prefer 'enum class' over 'enum' (Enum.3). Library uses unscoped enum flag bitmasks by design.
#endif
#endif
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 31.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    RenderTextClipped(text_pos, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    float adjustment = 20.0f; // Adjust this value based on how much shorter you want the scrollbar to be
    if (axis == ImGuiAxis_Y) {
    ...
    bb.Expand(ImVec2(-ImClamp(IM_FLOOR((bb_frame_width - 1.0f) * 0.5f), 0.0f, 3.0f), -ImClamp(IM_FLOOR((bb_frame_height - 2.0f) * 0.5f), 0.0f, 3.0f)));
    ...
    // Draw the background and grab to the ForegroundDrawList to ensure it's on top
    ImDrawList* draw_list = window->DrawList; // or use window->ForegroundDrawList if you need it specifically on top
    draw_list->AddRectFilled(bb_frame.Min, bb_frame.Max, bg_col, window->WindowRounding, flags);
    ...
    draw_list->AddRectFilled(grab_rect.Min, grab_rect.Max, grab_col, style.ScrollbarRounding);
...
void DrawFadingRect(ImDrawList* draw_list, const ImRect& check_bb, float pad, const ImColor& color, float t)
{
    ...
    for (int i = 0; i < steps; ++i)
    {
        ...
        float y_start = check_bb.Min.y + pad + 1 + ((check_bb.Max.y - check_bb.Min.y - 2 * pad - 2) * (i / (float)steps));
        float y_end = check_bb.Min.y + pad + 1 + ((check_bb.Max.y - check_bb.Min.y - 2 * pad - 2) * ((i + 2) / (float)steps));
        draw_list->AddRectFilled(ImVec2(check_bb.Min.x + pad + 1, y_start), ImVec2(check_bb.Max.x - pad - 1, y_end), ImColor(color.Value.x, color.Value.y, color.Value.z, alpha / 255.0f), 0);
    }
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
       ...
       // Create a gradient-filled rectangle
        ...
        for (int i = 0; i < steps; ++i)
        {
            ...
            window->DrawList->AddRectFilledMultiColor(
                ImVec2(frame_bb.Min.x, y1),
                ImVec2(frame_bb.Max.x, y2),
                color_start,
                color_start,
                color_end,
                color_end
        ...
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(0, 0, 0, 100), 2, popup_open ? ImDrawFlags_::ImDrawFlags_RoundCornersTop : ImDrawFlags_::ImDrawFlags_RoundCornersAll);
    }
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 3), text_col, popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.6f);
    }
    ...
    //  if (label_size.x > 0 && text)
        //  window->DrawList->AddText(ImVec2(window->DC.CursorPos.x + 8, window->DC.CursorPos.y - 28), ImColor(250, 250, 250, 200), label);
    Spacing();
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 65, 0.0f), ImVec2(window->Size.x - 3, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Render grab Slider Color using main_color
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 8), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 5 - 7), ImColor(
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 0 - CalcTextSize(value_buf).x, window->DC.CursorPos.y - 29), ImColor(255, 255, 255, static_cast<int>(current_alpha)), value_buf, value_buf_end); // Adjusted Y-coordinate

    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(window->DC.CursorPos.x + 32, window->DC.CursorPos.y - 29), ImColor(255, 255, 255, static_cast<int>(current_alpha)), label);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
   ...
   // if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
    //   hovered = true;
   // ImColor col = ((selected && hovered) ? ImColor(250, 192, 192) : hovered ? ImColor(112, 146, 190, 100) : ImColor(112, 146, 190));
    //    if (hovered || selected)
    //    {       
   ...
   //     }
    ...
    ImColor t_col = ((selected && hovered) ? ImColor(210, 100, 185) : hovered ? ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.5f) : selected ? ImColor(210, 100, 185) : ImColor(255, 255, 255, 100));
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // **New Code for Clipping: Check if the tab is within the visible area before rendering**
    const bool is_tab_visible = bb.Max.x >= tab_bar->ScrollingRectMinX && bb.Min.x <= tab_bar->ScrollingRectMaxX;
    if (!is_tab_visible)
    {
    ...
    if (want_clip_rect)
        PushClipRect(ImVec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX), bb.Min.y - 1000), ImVec2(tab_bar->ScrollingRectMaxX, bb.Max.y), true);
    ...
    // Select with right mouse button. This is so the common idiom for context menu automatically highlights the current widget.

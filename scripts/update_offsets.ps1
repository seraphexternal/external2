...
for ($i = 0; $i -le $MaxRetries; $i++) {
    try {
        ...
        if ($response.StatusCode -eq 200 -and $response.Content.Length -gt 500) {
            ...
            if ($content -match 'Roblox Version\s*:\s*(version-[0-9a-fA-F]+)') {
                $version = $Matches[1]
    ...
    namespace FFlags {
          inline uintptr_t GameNetCompressionLodByteBudgetThresholdPct = 0x7a90cb0;
          inline uintptr_t PhysicsSenderMaxBandwidthBps = 0x7a8e900;
          inline uintptr_t NextGenReplicatorEnabledWrite4 = 0x7eeb8f0;
    }
